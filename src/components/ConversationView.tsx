...
const ConversationView: React.FC<Props> = ({ transcripts, onWordSelect }) => {
    ...
    const renderText = (text: string) => {
        // Split by newlines to preserve paragraphs/formatting
        return text.split('\n').map((line, lineIndex) => {
            // Skip empty lines if they are consecutive, but keep one for spacing
            if (line.trim() === '') {
            ...
            return (
                <div key={lineIndex} style={{ marginBottom: '5px' }}>
                    {line.split(' ').map((word, wordIndex) => (
                        <span key={wordIndex} onClick={() => handleWordClick(word)} style={onWordSelect ? {cursor: 'pointer'} : {}}>
                            {word}{' '}
                        </span>
                    ))}
                </div>
                ...
                <div key={index} style={styles.transcriptLine}>
                    {t.speaker === 'system' ? (
                        ...
                        <div style={{...styles.bubble, ...(t.speaker === 'user' ? styles.userBubble : styles.modelBubble)}}>
                            ...
                            <div style={styles.text}>{renderText(t.text)}</div>
